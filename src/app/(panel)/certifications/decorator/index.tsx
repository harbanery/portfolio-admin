...
import { modalBodyProps } from "@/features/admin/utils/modal";
import { asAppError } from "@/features/admin/utils/error";
import { getImagesArray, type UploadFileLike } from "@/features/admin/utils/image";
import { useLocale } from "@/components/i18n/LocaleProvider";
import { skillsOptions } from "@/features/admin/utils/skills";
...
const CertificationDecorator = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(
        editingItem
          ? `/api/admin/certifications/${editingItem.id}`
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch("/api/admin/certifications/import", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  /** URL proxy file (delivery PDF/DOCX via /api/admin/file). */
  const buildProxyUrl = (url: string, download: boolean, name?: string) => {
    const params = new URLSearchParams({ url });
    if (download) params.set("download", "1");
    if (name) params.set("name", name);
    return `/api/admin/file?${params.toString()}`;
  };
