...
import { menuProjectType, menuRole } from "@/features/admin/utils/menu";
import { skillsOptions } from "@/features/admin/utils/skills";
import { getGithubRepoName } from "@/utils/helpers";
import { getImageString, getImagesArray } from "@/features/admin/utils/image";
import SmartImage from "@/features/admin/components/ui/smart-image";
...
const ProjectDecorator = ({ formLayout }: { formLayout: FormLayout[] }) => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const payload = await toPayload(values);
      const response = await fetch(`/api/admin/projects/${selectedItem!.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
